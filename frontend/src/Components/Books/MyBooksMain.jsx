...
const MyBooksMain = () => {
  ...
  const { favoriteLocal, favoriteLoading, isLogged } = useContext(AuthContext);

 

  useEffect(() => {
    if (favoriteLoading || !isLogged) return;

    const fetchBooks = async () => {
      try {
        const promises = favoriteLocal.map(async (fav) => {
          try {
            const book = await APIBooksId(fav);
            return book;
          } catch (error) {
            console.error(`Erro ao buscar livro ${fav}`, error);
            return null;
          }
        });
    ...
    fetchBooks();
  }, [reload, favoriteLoading, isLogged]);

  useEffect(() => {
    if (books.length === 0) return;

    setBooks((prevBooks) => prevBooks.filter((book) => favoriteLocal.includes(book.id)));
  }, [favoriteLocal, books.length]);
