  ...
  return (
    <span onClick={toggleFavorite}>
      {button ? (
        ...
        <StarIcon
          className={`absolute -right-5 -top-3 w-7 h-7 text-yellow-400 cursor-pointer  hover:scale-[115%] transition-all ${'bounceOnce'}`}
        />
      ) : (
        <StarIconOutline
          className={`absolute -right-5 -top-3 w-7 h-7 text-yellow-600  stroke-2 cursor-pointer  hover:scale-[115%] transition-all ${'bounceOnce'}`}
        />
      )}
    </span>
  )
